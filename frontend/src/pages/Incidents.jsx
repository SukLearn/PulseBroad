...
export default function Incidents() {
  const [range, setRange] = useState('7d'); const [serviceId, setServiceId] = useState(''); const [incidents, setIncidents] = useState(null); const [services, setServices] = useState([]);
  useEffect(() => { api.services().then(setServices); }, []);
  useEffect(() => { setIncidents(null); api.incidents(range, serviceId).then(setIncidents); }, [range, serviceId]);
  return <section><PageHeader eyebrow="Availability log" title="Incident history" description="Exact downtime intervals, recovery times, and ongoing outages in Tbilisi time." />
    ...
    <div className="panel">{incidents ? <IncidentTable incidents={incidents} /> : <Loading />}</div>
  </section>;
}
